...
const TagSync: React.FC = () => {
  ...
  const handleSync = () => {
    ...
    setIsLoading(true);
    clearLogs();
    addLog({ type: 'INFO', message: 'Starting tag sync...' });
    setTimeout(() => {
      addLog({ type: 'INFO', message: 'Updating tags for SKU-123 to "coffee, whole-bean, arabica".' });
    }, 500);
     setTimeout(() => {
      addLog({ type: 'INFO', message: 'Updating tags for SKU-456 to "coffee, ground, robusta".' });
    }, 1000);
    setTimeout(() => {
      addLog({ type: 'SUCCESS', message: 'Tag sync complete.' });
      setIsLoading(false);
    }, 1500);
  };
            ...
            <Card title="Sync Controls">
                <div className="space-y-3">
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                        This is a simulation of a product tag sync process.
                    </p>
                    <button
                        onClick={handleSync}
                        disabled={isLoading}
                    ...
                    >
                        {isLoading ? 'Syncing...' : 'Start Tag Sync'}
                    </button>
                </div>
            </Card>
        </div>
